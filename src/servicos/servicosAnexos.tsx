...
export const verificarSeAnexoJaExiste = async (nomeTabela: string, idEntidade: string, idCliente: number, arquivosObj: any) => {
    let formData = new FormData();
    paraCadaChaveNoObjetoExecutar((chave: any) => {
        formData.append("Arquivos", arquivosObj[chave]);
    }, arquivosObj)
    ...
    formData.append("idEntidade", `${idEntidade}`)
    formData.append("nomeTabela", `${nomeTabela}`)
    const conteudo = await fetch(url + "/Verificar", {
        method: 'POST',
        body: formData
    });
    const json: { existente: boolean } = await conteudo.json();
    return json;
}
